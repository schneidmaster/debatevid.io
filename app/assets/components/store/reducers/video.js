...
const videoReducer = (state, action) => {
  switch (action.type) {
    case "HYDRATE_VIDEO":
      ...
      const affDebaterOne = new Debater(
        Object.assign(videoData.affTeam.debaterOne, { school: affSchool })
      );
      let affDebaterTwo;
      if (videoData.affTeam.debaterTwo) {
        affDebaterTwo = new Debater(
          Object.assign(videoData.affTeam.debaterTwo, { school: affSchool })
        );
      }
      const negDebaterOne = new Debater(
        Object.assign(videoData.negTeam.debaterOne, { school: negSchool })
      );
      let negDebaterTwo;
      if (videoData.negTeam.debaterTwo) {
        negDebaterTwo = new Debater(
          Object.assign(videoData.negTeam.debaterTwo, { school: negSchool })
        );
      }
      ...
      const video = new Video(
        Object.assign(videoData, {
          ...
          tags: List(
            videoData.tagsVideos.map(tagVideo => tags.get(tagVideo.tagId))
          ),
          user: new User(videoData.user),
          affTeam: new Team({
            school: affSchool,
            debaterOne: affDebaterOne,
            debaterTwo: affDebaterTwo
          }),
          negTeam: new Team({
            school: negSchool,
            debaterOne: negDebaterOne,
            debaterTwo: negDebaterTwo
          })
        })
      );
    ...
    case "CREATED_TAG":
      return state
        .set("adding", false)
        .set("tagInput", "")
        .updateIn(["video", "tags"], tags =>
          tags.push(new Tag(action.payload))
        );
    default:
      return state;
  }
