      ...
      {segments.map((segment, idx) => {
        return (
          <div className="segment" key={segment.key}>
            <a href={segment.getLink()} target="_blank" rel="noreferrer">
              {segment.title}
            </a>
            ...
            <Glyphicon
              ...
              onClick={e => deleteFormSegment(idx)}
            />
          </div>
        );
      })}

      {renderIf(!hasSegments)(<p>Enter link to video from YouTube or Vimeo</p>)}
      ...
      {renderIf(!hasSegments || firstSegment.provider === "youtube")(
        <FormGroup>
          <InputGroup>
            ...
            />
            <InputGroup.Button>
              <Button bsStyle="primary" onClick={addFormSegment}>
                Add
              </Button>
            </InputGroup.Button>
          </InputGroup>
          {renderIf(!hasSegments)(
            <p className="help-block">
              If the video is in multiple segments on YouTube, you can add each
              link and DebateVid.io will automatically display as a playlist.
              Only one segment is supported for Vimeo.
            </p>
          )}
        </FormGroup>
      )}
